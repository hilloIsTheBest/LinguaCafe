        ...
        >
            ...
            <v-card-text>
                <template v-if="importResult === null || importResult.error">
                    ...
                    <!-- Csv file -->
                    <label class="font-weight-bold">Vocabulary file</label>
                    <v-file-input
                        v-model="importFile"
                        filled
                        dense
                        rounded
                        persistent-hint
                        hint="Accepted formats: .csv, .apkg"
                        ref="importFile"
                        accept=".csv,.apkg"
                    ...
                    <!-- Delimiter -->
                    <label class="font-weight-bold" v-if="!isApkg">Delimiter</label>
                    <v-text-field 
                        v-if="!isApkg"
                        v-model="delimiter"
                        filled
    ...
    export default {
            ...
            return {
                ...
                rules: {
                    importFileRule: (value) => {
                        ...
                        if (extension !== 'csv' && extension !== 'apkg') {
                            this.importFileValid = false;
                            return 'The selected file must be a .csv or .apkg file.';
                        }
        ...
        computed: {
            isApkg() {
                ...
                return name.toLowerCase().endsWith('.apkg');
            }
        },
        mounted: function() {
        },
        methods: {
            importFromCsv() {
                ...
                const endpoint = this.isApkg ? '/vocabulary/import-from-anki-apkg' : '/vocabulary/import-from-csv';
