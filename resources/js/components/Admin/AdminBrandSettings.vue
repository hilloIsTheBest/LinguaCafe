...
<script>
export default {
  data: () => ({ siteTitle: '', siteIconUrl: '', saving: false }),
  mounted() { this.load(); },
  methods: {
    load() {
      axios.post('/settings/global/get', { settingNames: ['siteTitle','siteIconUrl']})
        .then(r => { this.siteTitle = r.data.siteTitle || ''; this.siteIconUrl = r.data.siteIconUrl || ''; });
    },
    save() {
      this.saving = true;
      axios.post('/settings/global/update', { settings: { siteTitle: this.siteTitle, siteIconUrl: this.siteIconUrl }})
