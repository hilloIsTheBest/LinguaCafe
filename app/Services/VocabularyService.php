...
class VocabularyService {
    ...
    public function createPhrase($userId, $language, $words, $stage, $reading, $translation, $languagesWithoutSpaces) {
        // Enforce saved words limit when creating a phrase directly in saved state
        if ($stage < 0) {
            $limit = \App\Models\User::where('id', $userId)->value('saved_words_limit');
            if ($limit && $limit > 0) {
                $savedWords = EncounteredWord::where('user_id', $userId)->where('stage', '<', 0)->count();
                $savedPhrases = Phrase::where('user_id', $userId)->where('stage', '<', 0)->count();
    ...
    public function importFromCsv($userId, $language, $fileName, $delimiter, $onlyUpdate, $skipHeader) {
        // Detect LingQ CSV and handle with a dedicated parser for better compatibility
        ...
        try {
            ...
            // If we cannot read the first line, fall back to default handler below
            $firstLine = '';
        }

        if (is_string($firstLine)) {
            $firstLineLower = mb_strtolower($firstLine);
            // A simple heuristic for LingQ vocabulary exports
            // Typical headers include: term,phrase,tag1,tag2,meaninglanguage1,meaning1,...
            if ((str_contains($firstLineLower, 'term,') && str_contains($firstLineLower, 'phrase,')) ||
                (str_contains($firstLineLower, 'meaninglanguage1') && str_contains($firstLineLower, 'meaning1')))
            {
                return $this->importFromLingqCsv($userId, $language, $fullPath, $onlyUpdate);
            }
        }
        ...
        $updatedWords = 0;
        $rejectedWords = 0;
        // Saved words limit pre-checks
        $savedWordsLimit = User::where('id', $userId)->value('saved_words_limit');
        $currentSavedWords = EncounteredWord::where('user_id', $userId)->where('stage','<',0)->count()
            + Phrase::where('user_id', $userId)->where('stage','<',0)->count();
        ...
        foreach($records as $index => $record) {
            ...
            // set stage, enforce limit proactively
            if (isset($record[5])) {
                $newStage = $stageMapping[$stage];
                if ($newStage < 0 && $savedWordsLimit && $savedWordsLimit > 0) {
                    if ($currentSavedWords + 1 > $savedWordsLimit) {
                        $rejectedWords ++;
                        continue;
                    }
                    $currentSavedWords ++;
                }
                $encounteredWord->setStage($newStage, true);
            }
    ...
    private function importFromLingqCsv($userId, $language, $fullPath, $onlyUpdate) {
        $languagesWithoutSpaces = config('linguacafe.languages.languages_without_spaces');

        DB::disableQueryLog();

        $reader = Reader::createFromPath($fullPath, 'r');
        // LingQ exports are comma separated; use header row for mapping
        $reader->setDelimiter(',');
        ...
        // Helper to choose the first non-empty meaning (optionally could filter by language)
        $pickTranslation = function(array $row) {
            // Find all keys like meaning1..meaning10 with their language variants
            // Prioritize the first non-empty meaning
            $index = 1;
            while ($index <= 10) {
                $meaningKey = 'meaning' . $index;
                $langKey = 'meaninglanguage' . $index;
                if (array_key_exists($meaningKey, $row)) {
                    $meaningVal = trim((string)($row[$meaningKey] ?? ''));
                    if ($meaningVal !== '') {
                        return $meaningVal;
                    }
                }
                // in some exports there may be fewer columns; stop when neither key exists
                if (!array_key_exists($meaningKey, $row) && !array_key_exists($langKey, $row)) {
                    break;
                }
                $index++;
            }
        ...
        foreach ($records as $row) {
            // Normalize header keys to lowercase for safer access
            $normalized = [];
            foreach ($row as $key => $val) {
                $normalized[mb_strtolower((string)$key)] = is_string($val) ? trim($val) : $val;
            }

            $term = trim((string)($normalized['term'] ?? ''));
            if ($term === '') {
                $rejectedWords++;
                continue;
            }

            $translation = $pickTranslation($normalized);

            $isPhrase = !in_array($language, $languagesWithoutSpaces, true) && str_contains($term, ' ');

            if ($isPhrase) {
                // Create/update phrase
                $words = preg_split('/\s+/', $term);
                // Try to find an existing phrase by exact words match for the same user/language
                $existing = \App\Models\Phrase
                    ::where('user_id', $userId)
                    ->where('language', $language)
                ...
                if ($existing) {
                    ...
                    // default stage 0 (unknown)
                    $this->createPhrase($userId, $language, $words, 0, '', $translation, $languagesWithoutSpaces);
                    $createdWords++;
                }
            } else {
                // Create/update single word
                $lowerTerm = mb_strtolower($term);
                if (mb_strlen($lowerTerm) >= 255) {
                    $rejectedWords++;
                    continue;
                }
                ...
                if (!$encounteredWord) {
                    ...
                    $encounteredWord->user_id = $userId;
                    $encounteredWord->language = $language;
                    $encounteredWord->word = $lowerTerm;
                    $encounteredWord->translation = '';
                    $encounteredWord->lemma = '';
                    $encounteredWord->base_word = '';
                    $encounteredWord->reading = '';
