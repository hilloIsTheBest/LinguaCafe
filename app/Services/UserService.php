...
class UserService {
    ...
    public function getUsers($userId) {
        $users = User
            ::select(['id', 'name', 'email', 'is_admin', 'password_changed', 'created_at', 'saved_words_limit'])
            ->get();

        foreach ($users as $user) {
            $user->created_at_text = Carbon::parse($user->created_at)->format('Y-m-d');
            $user->is_current_user = $user->id === $userId;
            $user->saved_words = EncounteredWord::where('user_id', $user->id)->where('stage', '<', 0)->count();
        }
    ...
    public function createUser($name, $email, $password, $isAdmin, $passwordChanged, $savedWordsLimit = 0) {
        ...
        $user->saved_words_limit = intval($savedWordsLimit);
        $user->password = Hash::make($password);
        $user->save();
    ...
    public function updateUser($userId, $name, $email, $isAdmin, $savedWordsLimit = null) {
        // check for duplicated e-email address
        $user = User
            ::where('email', $email)
            ->where('id', '<>', $userId)
