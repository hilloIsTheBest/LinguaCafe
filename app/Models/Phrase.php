...
class Phrase extends Model
{
    ...
    public function setStage($stage) {       
        ...
        if ($this->stage >= 0 && $stage < 0) {
            ...
            if ($limit && $limit > 0) {
                $savedWords = \App\Models\EncounteredWord::where('user_id', $this->user_id)->where('stage', '<', 0)->count();
                $savedPhrases = Phrase::where('user_id', $this->user_id)->where('stage', '<', 0)->count();
