...
class SettingsController extends Controller
{
    ...
    // Uploads a brand icon and updates the global siteIconUrl setting
    public function uploadBrandIcon(Request $request) {
        ...
        $path = $file->store('public/images/branding');
        $url = Storage::url($path);

        try {
            $this->settingsService->updateGlobalSettings(['siteIconUrl' => $url]);
        } catch (\Exception $e) {
            abort(500, $e->getMessage());
        }
