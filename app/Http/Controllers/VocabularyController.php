...
use App\Http\Requests\Vocabulary\ImportFromAnkiRequest;
...
class VocabularyController extends Controller {
    ...
    public function importFromAnkiApkg(ImportFromAnkiRequest $request) {
        $userId = Auth::user()->id;
        $language = Auth::user()->selected_language;
        $importFile = $request->file('importFile');
        $onlyUpdate = $request->post('onlyUpdate');
        ...
        try {
            $fileName = $this->tempFileService->moveFileToTempFolder($userId, $importFile);
            $importResponseData = (new \App\Services\AnkiImportService())->importApkg($userId, $language, $fileName, $onlyUpdate);
        } catch (\Exception $e) {
            if (isset($fileName)) {
                $this->tempFileService->deleteTempFile($fileName);
            }
